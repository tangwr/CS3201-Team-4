	...
	public:
		...
		TEST_METHOD(getQuery)
		{
			...
			queries.push_back("assign a;variable b; Select a such that Uses(a, \"x\")");	
				...
				QueryTree qt = qp.getQuery();
				Assert::AreEqual(1, (int)qt.getLimits().size());
				Assert::IsTrue("a" == qt.getLimits().at(0)->getLeftChild());
				Assert::IsTrue("x" == qt.getLimits().at(0)->getRightChild());
				Assert::IsTrue(ASSIGN == qt.getLimits().at(0)->getLeftChildType());
				Assert::IsTrue(STRINGVARIABLE == qt.getLimits().at(0)->getRightChildType());
		    
				queries.push_back("assign a; stmt s; Select s such that Follows(1,5)");
				QueryParser qp2;
				qp2.isValid(queries.at(1));
				QueryTree qt2 = qp2.getQuery();
				Assert::AreEqual(1, (int)qt2.getUnLimits().size());
