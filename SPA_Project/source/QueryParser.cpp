...
void QueryParser::getDeclare(QueryTree *qt) {
	...
	string nextToken;
	do {
		cout << "\n the tokenDo is: " << tokenizer->peekToken();
...
void QueryParser::getSelect(QueryTree *qt) {
	cout << "\n the selectFirst is: " << tokenizer->peekToken();
	tokenizer->getToken();
	cout << "\n the select2nd is: " << tokenizer->peekToken();
	...
	if (token.compare(SYMBOL_TUPLE_OPEN_BRACKET) == EQUAL) {
		...
		string nextToken;
		do {
			cout << "\n the tokenDoSelect is: " << tokenizer->peekToken();
			string selectName = getSelectSyn();
			qt->insertSelect(varMap[selectName]);
			nextToken = tokenizer->getToken();
		} while (nextToken.compare(SYMBOL_COMMA) == EQUAL);

		match(nextToken, SYMBOL_TUPLE_CLOSE_BRACKET);
	}
	else {
		if (isVarNameExists(token)) {
...
void QueryParser::getSuchThat(QueryTree *qt) {
	...
	cout << "\n the and  is : " << tokenizer->peekToken();
...
void QueryParser::getModifies(QueryTree* qt) {
	...
	cout << "\n the testing is : " << tokenizer->peekToken();
	...
	cout << "\n the cur quote is : " << tokenizer->peekToken();
	...
	if (isVarNameExists(leftChild.getParaName())) {
		m->insertSynList(leftChild);
	}
	if (isVarNameExists(rightChild.getParaName())) {
		m->insertSynList(rightChild);
	}
	qt->insertResult(m);
}
void QueryParser::getUses(QueryTree* qt) {
	...
	if (isVarNameExists(leftChild.getParaName())) {
		u->insertSynList(leftChild);
	}
	if (isVarNameExists(rightChild.getParaName())) {
		u->insertSynList(rightChild);
	}
	qt->insertResult(u);
}
void QueryParser::getCalls(QueryTree* qt) {
	...
	if (isVarNameExists(leftChild.getParaName())) {
		c->insertSynList(leftChild);
	}
	if (isVarNameExists(rightChild.getParaName())) {
		c->insertSynList(rightChild);
	}
	qt->insertResult(c);
}
void QueryParser::getCallsStar(QueryTree* qt) {
	...
	if (isVarNameExists(leftChild.getParaName())) {
		cs->insertSynList(leftChild);
	}
	if (isVarNameExists(rightChild.getParaName())) {
		cs->insertSynList(rightChild);
	}
	qt->insertResult(cs);
}
void QueryParser::getParent(QueryTree* qt) {
	...
	if (isVarNameExists(leftChild.getParaName())) {
		p->insertSynList(leftChild);
	}
	if (isVarNameExists(rightChild.getParaName())) {
		p->insertSynList(rightChild);
	}
	qt->insertResult(p);
}
...
void QueryParser::getAffects(QueryTree* qt) {
	...
	if (isVarNameExists(leftChild.getParaName())) {
		a->insertSynList(leftChild);
	}
	if (isVarNameExists(rightChild.getParaName())) {
		a->insertSynList(rightChild);
	}
	qt->insertResult(a);
}
void QueryParser::getAffectsStar(QueryTree* qt) {
	...
	if (isVarNameExists(leftChild.getParaName())) {
		as->insertSynList(leftChild);
	}
	if (isVarNameExists(rightChild.getParaName())) {
		as->insertSynList(rightChild);
	}
	qt->insertResult(as);
}
void QueryParser::getPattern(QueryTree *qt) {
	...
	if (lcType == ASSIGN || lcType == WHILE) {
		...
		if (lcType == WHILE) {
			...
			if (fact.compare(SYMBOL_UNDERSCORE) == EQUAL && close.compare(SYMBOL_PATTERN_CLOSE) == EQUAL) {
				...
				cout << "\n the underScore exp is : " << tokenizer->peekToken();
				...
				factor.setParaName(exp);
				factor.setParaType(STRINGVARIABLE);
				cout << "\n now the token: " << tokenizer->peekToken();
				tokenizer->getToken();
			}
			else {
				throwError(ERROR_FACTOR);
		...
		cout << "\n the 2nd comma : " << tokenizer->peekToken();
	...
	if (isVarNameExists(factor.getParaName())) {
		p->insertSynList(factor);
	}
	qt->insertResult(p);
...
void QueryParser::getWith(QueryTree *qt) {
	...
	if (isVarNameExists(lc)) {
		...
		cout << "\n the with is : " << tokenizer->peekToken();
		string proceName = tokenizer->getToken();
		match(proceName, IDENT);
		leftChild.setParaName(proceName);
		leftChild.setParaType(STRINGVARIABLE);
	...
	cout << "\n the now with is : " << tokenizer->peekToken();
...
void QueryParser::match(string token, string matchRe) {
	cout << "\n the tokenPeek is: " << tokenizer->peekToken();
