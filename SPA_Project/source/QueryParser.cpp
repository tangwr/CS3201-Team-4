...
bool QueryParser::isValid(string query) {
	...
	counter2 = 0;
	checkFirstLeftChild = "";
	checkFirstRightChild = "";
	checkSecondLeftChild = "";
	checkSecondRightChild = "";
	queryTree = new QueryTree();
	string q = trim(query);
	cout<< "the trim query is: " <<q;
	queryTree->setIsComonVar(false);
	varMap.clear(); // clear the varMap
	queryTree->getComonVar().clear(); // clear the commonVar map
			...
			if (i == 0) {
				if (isVarNameExists(variable.at(0))) {
					...
					if (isSuchThanBeforePattern) {
						...
						if (leftChild.compare(checkFirstRightChild) == 0 && !checkFirstRightChild.empty()) {
							counter1++;
						}
					}
					if (queryTree->getSelect().count(leftChild)) {
				...
				if (isVarNameExists(variable.at(1))) {
					...
					if (isSuchThanBeforePattern) {
						...
						if (rightChild.compare(checkFirstLeftChild) == 0 && !checkFirstLeftChild.empty()) {
							counter2++;
						}
